...
// Cars selection : top 6 toutes villes (events Riviera-wide)
const carsForEvent = computed(() => {
  if (service.value !== 'cars') return [];
  return RENTAL_CARS
    .filter((c) => c.badges?.includes('flagship') || c.badges?.includes('popular'))
    .slice(0, 6);
});
      ...
      <div class="max-w-[1600px] mx-auto px-6 sm:px-12 py-12 sm:py-14">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <NuxtLink
            ...
            :key="e.slug"
            :to="localePath(`/services/${service}/event/${e.slug}`)"
            class="popular-chip group"
          >
            <p class="text-[10px] uppercase tracking-[0.18em] text-misana-muted mb-1.5">
              {{ serviceName }}
