...
const SECTIONS = computed(() => [
  { items: ESTABLISHMENTS_REF.value.filter((e) => e.category === 'restaurant').sort(sortByCity), ns: 'restaurants', cat: 'restaurant' },
  { items: ESTABLISHMENTS_REF.value.filter((e) => e.category === 'beach-club').sort(sortByCity), ns: 'beachClubs', cat: 'beach-club' },
  { items: ESTABLISHMENTS_REF.value.filter((e) => e.category === 'palace'), ns: 'palaces', cat: 'palace' },
  { items: ESTABLISHMENTS_REF.value.filter((e) => e.category === 'nightclub').sort(sortByCity), ns: 'nightlife', cat: 'nightclub' },
]);

// State local d'expansion par section (cle = ns). Utilise pour le bouton
// "Voir les autres ({n})" : truncation a 6 par defaut, full sur clic.
const VISIBLE_LIMIT = 6;
const expandedSections = ref<Record<string, boolean>>({});
function visibleItems<T>(block: { ns: string; cat: string; items: T[] }): T[] {
  // Palaces : tout afficher (hors scope, pas de truncation)
