  ...
  if (scenarioId === 'vehicle' && compat.vehicle) {
    try {
      ...
      if (car.value) {
        ...
        backLink = localePath({ name: 'voitures-brandModel', params: { brandModel: compat.vehicle } });
  ...
  if (scenarioId === 'access' && compat.establishment) {
    try {
      ...
      if (establishment.value) {
        ...
        backLink = localePath({ name: 'reservations-establishment', params: { establishment: compat.establishment } });
      }
    } catch {}
  }
  ...
  if (scenarioId === 'event') {
    ...
    if (slug) {
      try {
        ...
        // Phase 2.2-fix : OR explicite pour matcher new ou legacy slug.
        const data = await (sanity.client as any).fetch(
          /* groq */ `*[_type == "event" && (slugI18n[$locale].current == $slug || slug.current == $slug)][0]{
